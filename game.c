...
void game(SDL_Surface *screen, const char *name, int client_number)
{
    ...
    while (playing)
    {
                    ...
                    switch(event.key.keysym.sym)
                    {
                        ...
                        case SDLK_UP:
                            ...
                            if (FD_ISSET(sock, &rdfs))
                            {
                                ...
                                printf("client : response gotten, pos_y = %d pos_x = %d\n", 
                                    game->player_infos[client_number].y_pos, game->player_infos[client_number].x_pos);
                                    moveCharacterTCP(map, character_list->character, bomb_list, UP);
                            }
                            break;
                        case SDLK_DOWN:
                            ...
                            if (FD_ISSET(sock, &rdfs))
                            {
                                ...
                                printf("client : response gotten, pos_y = %d pos_x = %d\n", 
                                    game->player_infos[client_number].y_pos, game->player_infos[client_number].x_pos);
                                    moveCharacterTCP(map, character_list->character, bomb_list, DOWN);
                            }
                            break;
                        case SDLK_RIGHT:
                            ...
                            if (FD_ISSET(sock, &rdfs))
                            {
                                ...
                                printf("client : response gotten, pos_y = %d pos_x = %d\n", 
                                    game->player_infos[client_number].y_pos, game->player_infos[client_number].x_pos);
                                moveCharacterTCP(map, character_list->character, bomb_list, RIGHT);
                            }
                            break;
                        case SDLK_LEFT:
                            ...
                            if (FD_ISSET(sock, &rdfs))
                            {
                                ...
                                printf("client : response gotten, pos_y = %d pos_x = %d\n", 
                                    game->player_infos[client_number].y_pos, game->player_infos[client_number].x_pos);
                                    moveCharacterTCP(map, character_list->character, bomb_list, LEFT);
                            }
                            break;
                        case SDLK_SPACE:
        ...
        if (character_list != NULL) {
            ...
            draw_characters_on_screenTCP(screen, &character_list, map,game,0);
        } else {
            playing = false;
        } 
        SDL_Flip(screen);
